...
edcrumble_init_log_interval_ms = 500;
...
function send_log() {
    ...
    if(log_slice.length > 0) {
        $.ajax({
            url: baseurl + 'action/lds/edcrumble/log_activity?log_session_id=' + log_session_id + '&guid=' + $('#lds_edit_guid').val() + '&revision=' + $('#lds_edit_revision').val(),
            method: 'POST',
...
function add_event_log(event) {
    ...
    for([key, item] of Object.entries(cloned_resourcesList)) {
        ...
        if(typeof cloned_content === "object")
            cloned_content['data'] = null;
